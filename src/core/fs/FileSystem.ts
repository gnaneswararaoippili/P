...
export class FileSystem {
  ...
  async rm(path: string, cwd: string, recursive: boolean = false): Promise<void> {
    const { parent, targetName } = resolveParentNode(path, cwd, this.root);
  ...
  async cp(srcPath: string, destPath: string, cwd: string, recursive: boolean = false): Promise<void> {
    ...
    if (targetParent.children.has(targetName) && targetParent.children.get(targetName)?.type === 'directory') {
  ...
  async mv(srcPath: string, destPath: string, cwd: string): Promise<void> {
